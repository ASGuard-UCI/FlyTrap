...
from flytrap.dev_runner_vogues import PhysicalAdversarialPatchRunnerAdaptive
...
def main(config):
    ...
    detector = builder.MODEL.build(cfg.detector)
    pose_estimator = builder.MODEL.build(cfg.pose_estimator)
    ...
    loss = builder.AdaptiveLossWrapper(cfg.loss, strategy='cv')
    ...
    runner = PhysicalAdversarialPatchRunnerAdaptive(
        model=model,
        detector=detector,
...
if __name__ == '__main__':
    ...
    argparse.add_argument('--config', type=str, default='config/finals/mixformer_cvt_position_engine_final_vogues_v3.py') # used for debug
