...
# dataset settings
img_keys = ['search', 'template', 'online_template', 'blend_template']
train_pipeline = [
    dict(type='TemplateSample', same_video=False, online_template=True, num_online=1, rand_blend=True),
    ...
    dict(type='BlendImage', alpha=0.5, prob=0.5),
    # dict(type='CustomNormalize', img_keys=img_keys, **img_config),
    dict(type='CustomCollect',
         keys=['search', 'template', 'online_template', 'coords', 'normalize_bbox'],
...
post_transform = [
    dict(type='CustomNormalize', img_keys=['search', 'template', 'online_template'], **model_img_config),
]
