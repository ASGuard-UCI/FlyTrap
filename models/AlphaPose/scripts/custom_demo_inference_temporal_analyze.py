...
### Customized script
### Save the heatmap output as the attack target
...
if __name__ == "__main__":
    ...
    ## Save heat map into pickle file
    import pickle
    heatmap_mse = []
    prev_heatmap = None
    ...
    try:
        for i in im_names_desc:
            ...
            with torch.no_grad():
                ...
                if prev_heatmap is not None:
                    mse = (torch.sqrt((hm - prev_heatmap) ** 2)).mean()
                    float_mse = mse.item()
                    heatmap_mse.append(float_mse)
                    prev_heatmap = hm
                else:
                    prev_heatmap = hm
        ...
        pickle.dump(heatmap_mse, open('heatmap_mse_adv.pkl', 'wb'))
        print_finish_info()
