...
class YOLODetector(BaseDetector):
    def __init__(self, cfg, opt=None):
        ...
        self.model_cfg = cfg.get('CONFIG', 'models/AlphaPose/detector/yolo/cfg/yolov3-spp.cfg')
        self.model_weights = cfg.get('WEIGHTS', 'models/AlphaPose/detector/yolo/data/yolov3-spp.weights')
    ...
    def images_detection(self, imgs, orig_dim_list):
        ...
        with torch.no_grad():
            ...
            orig_dim_list = torch.index_select(orig_dim_list.cuda(), 0, dets[:, 0].long())
    ...
    def images_detection_adv(self, imgs, orig_dim_list):
        """
        Based on ``images_detection``, this function is used for adversarial attack
        """
        args = self.detector_opt
        _CUDA = True
        if args:
        ...
        if not self.model:
            self.load_model()
        # with torch.no_grad():
        # Modification 1: remove the no_grad() context manager to enable gradient computation
        
        # Modifcation 2: the image is already pre-processed, this will cut off the gradient computation
        # imgs = imgs.to(args.device) if args else imgs.cuda()
