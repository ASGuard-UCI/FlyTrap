...
class MixFormerOnline(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        ## for debug
        # cv2.imwrite(f'debug2/{self.frame_id:04d}.jpg', x_patch_arr)
        ...
        # since we already sigmoid in the track_head, we do not need to sigmoid here
        ...
        if self.debug:
            ...
            image = cv2.rectangle(image.copy(), (int(x1),int(y1)), (int(x1+w),int(y1+h)), color=(0,0,255), thickness=2)
