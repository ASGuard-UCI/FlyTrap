...
class MixFormerOnlineScore(nn.Module):
    ...
    def forward_head(self, search, template, run_score_head=True, gt_bboxes=None):
        ...
        if run_score_head:
            ...
            # (b,c,h,w) --> (b,h,w)
            out_dict.update({'pred_scores': self.score_branch(search, template, gt_bboxes).view(-1).sigmoid()})
...
class MixFormerOnlineScore_RandomScoreHead(nn.Module):
    """ Mixformer tracking with score prediction module, whcih jointly perform feature extraction and interaction. 
    Modify:
        - Randomize the bbox send to the score head.
        - Forward the score head multiple times.
    """
    def __init__(self, backbone, box_head, score_branch=None, head_type="CORNER"):
        """ Initializes the model.
        """
    ...
    def _randomize_bbox(self, bbox):
        ...
        random_value = torch.rand(new_h.shape, device=bbox.device) * 2 - 1
        shift_x = (new_w * random_value)
        shift_y = (new_h * random_value)
    ...
    def forward_head(self, search, template, run_score_head=True, gt_bboxes=None):
        ...
        out_dict.update(out_dict_box)
        if run_score_head:
            # TODO: [ ] hard code the randomize bbox parameter here
            # forward the classification head
            if gt_bboxes is None:
                # multiple forward the score head
                avg_scores = 0
                multi_forward = 3
                for i in range(multi_forward):
                    outputs_coord_ = self._randomize_bbox(outputs_coord)
                    gt_bboxes = box_cxcywh_to_xyxy(outputs_coord_.clone().view(-1, 4)) # (b,c,h,w) --> (b,h,w)
                    avg_scores += self.score_branch(search, template, gt_bboxes).view(-1).sigmoid()
                out_dict['pred_scores'] = avg_scores / multi_forward
            else:
...
def build_mixformer_cvt_online_score_randomhead(cfg, settings=None, train=True):
    backbone = get_mixformer_online_model(cfg)  # backbone without positional encoding and attention mask
    box_head = build_box_head(cfg)  # a simple corner head
    score_branch = ScoreDecoder(pool_size=4, hidden_dim=cfg.MODEL.HIDDEN_DIM, num_heads=cfg.MODEL.HIDDEN_DIM//64)  # the proposed score prediction module (SPM)
    model = MixFormerOnlineScore_RandomScoreHead(
        backbone,
        box_head,
        score_branch,
        head_type=cfg.MODEL.HEAD_TYPE
