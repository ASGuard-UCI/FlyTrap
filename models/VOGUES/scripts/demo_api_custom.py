...
class DetectionLoader():
    ...
    def image_detection(self, manual_dets=None):
        imgs, orig_imgs, im_names, im_dim_list = self.image
        if imgs is None:
            self.det = (None, None, None, None, None, None, None)
            return

        if manual_dets is not None:
            dets = manual_dets
            boxes = dets[:, 1:5]
            scores = dets[:, 5:6]
...
class SingleImageAlphaPose():
    ...
    def process(self, im_name, image, manual_bbox=None):
        ...
        try:
            start_time = getTime()
            with torch.no_grad():
                (inps, orig_img, im_name, boxes, scores, ids, cropped_boxes) = self.det_loader.process(im_name, image, manual_bbox).read()
                if orig_img is None:
