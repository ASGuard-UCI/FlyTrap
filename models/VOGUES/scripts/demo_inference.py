...
if __name__ == "__main__":
    ...
    if args.save_video and mode != 'image':
        ...
        if mode == 'video':
            ...
            outputfile = os.path.splitext(os.path.basename(input_source))[0] + '.json'
        else:
            video_save_opt['savepath'] = os.path.join(args.outputpath, 'AlphaPose_webcam' + str(input_source) + '.mp4')
            outputfile = 'webcam' + str(input_source) + '.json'
        video_save_opt.update(det_loader.videoinfo)
        writer = DataWriter(cfg, args, save_video=True, video_save_opt=video_save_opt, queueSize=queueSize, outputfile=outputfile).start()
    else:
        if mode == 'video':
            outputfile = os.path.splitext(os.path.basename(input_source))[0] + '.json'
        writer = DataWriter(cfg, args, save_video=False, queueSize=queueSize, outputfile=outputfile).start()
