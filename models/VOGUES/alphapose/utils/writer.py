...
class DataWriter():
    def __init__(self, cfg, opt, save_video=False,
                 ...
                 queueSize=1024, outputfile=None):
        self.cfg = cfg
        self.opt = opt
    ...
    def update(self):
        ...
        while True:
            ...
            if orig_img is None:
                ...
                if self.save_video:
                    stream.release()
                write_json(final_result, self.opt.outputpath, form=self.opt.format, for_eval=self.opt.eval, outputfile=self.outputfile)
                print(f"Results have been written to json: {self.outputfile}\n")
                return
