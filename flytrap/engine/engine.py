...
class PhysicalEngine:
    def __init__(self, 
                 ...
                 size=0.4,
                 out_size=320):
        """Physical engine for simulate the physical world in closed loop drone control
        Args:
            render: 
            history: TODO
            ...
            human_ratio (float): human height / human width
    ...
    def adv_target_bbox_generator(self, bbox, distance):
        ...
        Args:
            ...
            """
        # assume the human size at first frame as the initial size
        # then based on it, move forward `interval_distance` each step
        initial_human_size = bbox[0] / self.obj_human_ratio
        target_distance = distance - self.interval_distance
        # clamp the distance to be positive
        target_distance = np.clip(target_distance, 0.5, None) # [self.evolve_step, B]
        
        # based on pin-hole camera model, the size of the object is inversely proportional to the distance
        shrink_rate = (target_distance / distance)
        
        # recursively shrink the human size
        recursive_shrink_rate = np.cumprod(shrink_rate, axis=0)
        
        # the target bbox size is abosulte size, because even the umbrella is closer and larger
        # the initial human size is stored in the tracking for distance reference
        # thus we want to inject the adversarial size based on the initial human size in the closed-loop frame simulation
        adv_target_bbox_size = initial_human_size * recursive_shrink_rate
    
        
        ## TODO: consider the human shape, currently is square
        ...
        # # stack into [self.evolve_step, B, 2] [h, w]
        # return np.stack([adv_target_bbox_height, adv_target_bbox_width], axis=2)
        
        # the above code is for human shape, we instead simply use square shape
        
        # check the adversarial target bbox size should be smaller than the umbrella size
        adv_target_bbox_size = np.minimum(adv_target_bbox_size, bbox)
        assert np.all(adv_target_bbox_size <= bbox)
        return np.stack([adv_target_bbox_size, adv_target_bbox_size], axis=2) # [self.evolve_step, B, 2] [h, w]
    ...
    def crop_search_area(self, image, umbrella_bbox, target_bbox, H, W):
        ...
        first_frame_state = umbrella_bbox[0] / np.array((1, 1, self.obj_human_ratio, self.obj_human_ratio))
        state_bbox = np.concatenate((first_frame_state[None, ], target_bbox[:-1]), axis=0) # [self.evolve_step, B, 4]
        # TODO: word around, manually set t1 bbox to be the center
        state_bbox[1, :, :2] = np.array([W / 2, H / 2])
        ...
        crop_bbox = _randomize_bbox(state_bbox, target_bbox, self.search_factor) # [self.evolve_step, B, 4] [cx, cy, w, h]
    ...
    def distance_renderer(self, patch, target_distance):
        ...
        for i in range(self.evolve_step):
            for j in range(B):
                distance_ = target_distance[i, j] 
                camera_args_eval = {'dist': distance_, 
                                    'elev': np.random.uniform(-5, 5) if distance_ > 5 else 0, 
                                    'azim': np.random.uniform(-5, 5) if distance_ > 5 else 0}
                rendered_patch.append(self.render(patch, camera_args_eval))
    ...
    def distance_applyer(self, images, bbox, patch):
        ...
        composed_images = torch.zeros((self.evolve_step, B, H, W, 3)).cuda()
        for e in range(evolve_step):
            for b in range(B):
                image_torch = torch.tensor(images[e, b], dtype=torch.float32).unsqueeze(0).cuda()
                composed_image_, _ = self.applyer(image_torch, patch[e * B + b], normalized_bbox[e, b][None, ])
    ...
    def __call__(self, images, bbox, patch):
        ...
        # this step compute the adversarial target bbox for each step to make the drone move forward `interval_distance`
        ...
        ## debug
        # debug_bbox1 = umbrella_bbox[1, 0].copy()
        # debug_bbox2 = target_shrink_bbox[1, 0].copy()
        # debug_img = distance_images[0, 1].copy()
...
class PhysicalEngineV2:
    def __init__(self, 
        ...
        """
        Modification:
            1.  Divide the distance into multiple stages, each stage use the same shrink area,
                this can make the adversarial patch continuous shrinking
            2.  Clip the search region based on the shrink area in the previous stage
        Args:
            render: 
            history: TODO
            evolve_step (int): number of steps to evolve the physical system
...
class PhysicalEngineV3:
    ...
    def distance_renderer(self, patch, target_distance):
        B = target_distance.shape[1]
        # use single patch for all the frames
        rendered_patch = [self.render(patch)] * (self.evolve_step * B)
...
class PhysicalEngineV5:
    def __init__(self, 
                 ...
                 shrink_rate = [0.05, 0.1, 0.2, 0.4],
                 search_factor=4.5,
                 first_frame_random_search_factor=[1,6],
                 size=0.4,
                 out_size=320,
                 det_out_size=608):
        """
        Modification:
            1.  Based on V4, prepare inputs for object detection and pose estimation for adaptive attack
        Args:
            render: 
            history: TODO
            evolve_step (int): number of steps to evolve the physical system
        ...
        self.out_size = out_size
        self.det_out_size = det_out_size
        self.first_frame_random_search_factor = first_frame_random_search_factor
    ...
    def adv_target_bbox_generator(self, bbox, distance):
        ...
        adv_target_bbox_size = np.minimum(adv_target_bbox_size, bbox)
        assert np.all(adv_target_bbox_size <= bbox)
        return np.stack([adv_target_bbox_size, adv_target_bbox_size], axis=2), shrink_rate # [self.evolve_step, B, 2] [h, w]
    ...
    def crop_search_area(self, image, umbrella_bbox, target_bbox, H, W):
        ...
        first_frame_random_search_factor = np.random.uniform(self.first_frame_random_search_factor[0], self.first_frame_random_search_factor[1])
        crop_bbox_first_frame = _randomize_bbox(umbrella_bbox[0:1], target_bbox[0:1], first_frame_random_search_factor)
    ...
    def prepare_pose_input(self, patch, shrink_rate):
        images_pose = test_transform_torch(patch, shrink_rate)
    ...
    def __call__(self, images, bbox, patch):
        ...
        # this step compute the adversarial target bbox for each step to based on distance stage threshold
        # The `target_shrink_bbox` is the absolute coordinate in the original image
        target_shrink_bbox, target_shrink_rate = self.adv_target_bbox_generator(target_bbox_size, target_distance) # [self.evolve_step, B, 2] [h, w]
        ...
        ### (V4 New!) Add the data preparation for object detection and pose estimation for adaptive attack
        det_input_image, new_umbrella_bbox, orig_shape = self.prepare_det_input(images, bbox, H, W)
        # TODO: prepare input for the pose estimation model
        # pose_input_image, pose_target_ht = self.prepare_pose_input(input_image, target_bbox_new, H, W)
        ...
        rendered_patch = self.distance_renderer(patch, target_distance)
        composed_images = self.distance_applyer(input_image, umbrella_bbox_new, rendered_patch) # [self.evolve_step, B, H, W, 3]
        composed_images_det = self.distance_applyer(det_input_image[None, ], 
                                                    new_umbrella_bbox[None], 
                                                    rendered_patch).squeeze() # [B, H, W, 3]
        
        # Prepare the input for pose estimation model
        # we don't need to rely on the background image, we can directly use the cropped patch itself
        # based on the target_shrink_area, and do some randomization
        # TODO: add randomization here, now we fix the area to be the same as the target_shrink_area
        composed_images_pose = self.prepare_pose_input(rendered_patch, torch.tensor(target_shrink_rate.reshape(-1)))
        
        # TODO: ugly workaround here
        composed_images_det = composed_images_det.permute(0, 3, 1, 2) / 255.
        
        # normalize the bbox to [0, 1]
        target_bbox_new = target_bbox_new / self.out_size
        umbrella_bbox_new = umbrella_bbox_new / self.out_size
        target_bbox_det = target_shrink_bbox[0].copy()
        
        target_bbox_new = torch.tensor(target_bbox_new).cuda() # [self.evolve_step, B, 4] [cx, cy, w, h]
        umbrella_bbox_new = torch.tensor(umbrella_bbox_new).cuda() # [self.evolve_step, B, 4] [cx, cy, w, h]
        target_bbox_det = torch.tensor(target_bbox_det).cuda() # [self.evolve_step, B, 4] [cx, cy, w, h]
        orig_shape = torch.tensor(orig_shape).cuda()
        
        return composed_images, umbrella_bbox_new, target_bbox_new, composed_images_det, target_bbox_det, orig_shape, composed_images_pose
    
    
@ENGINE.register_module()
class PhysicalEngineV6:
    def __init__(self, 
                 render: dict = None, 
                 applyer: dict = None,
                 history=None, 
        ...
        """
        Modification:
            1.  Based on V5, support `evolve_step=1` for ablation study
        Args:
            render: 
            history: TODO
            evolve_step (int): number of steps to evolve the physical system
