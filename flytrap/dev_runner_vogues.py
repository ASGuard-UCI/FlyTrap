...
## We plan to improve the optimization process to better search for adversarial examples
...
class PhysicalAdversarialPatchRunnerAdaptive:
    ...
    def __init__(self,
                 ...
                 pose_estimator,
                 target_pose,
                 applyer, # used for eval
                 renderer, # used for eval
                 tracker,
                 engine,
                 work_dir: str,
                 config: dict,
                 ...
                 alarm: Callable = None, # for spatial-temporal defense
                 eval_interval: int = 1,
                 log_cfg: dict = None,
                 debug: bool = False):
        ...
        self.target_pose = self._pose_heatmap(target_pose)
        self.applyer = applyer  # used for evaluation
        ...
        if alarm is not None:
            self.alarm = builder.ALARM.build(alarm)
        else:
            self.alarm = None
    ...
    def _pose_heatmap(self, target_pose):
        """Convert pose heatmap to discrete GT."""
        import pickle
        # Load the predicted heatmap (C x H x W)
        heatmap = pickle.load(open(target_pose, 'rb'))
    ...
    def train_epoch(self, epoch: int):
        ...
        with tqdm(total=len(self.train_loader), desc="Training") as pbar:
            for batch_idx, data in enumerate(self.train_loader):
                ...
                # input_search is the search image after applying physical engine, with rendered adversarial patch
                # gt_bbox is the bbox of the umbrella in the search image
                # target_bbox is the bbox of the adversarial target in the search image
                input_search, gt_bbox, target_bbox, input_det, target_bbox_det, orig_shape, composed_images_pose = self.engine(data['search'], 
                                                                                             data['umbrella_bbox'], 
                                                                                             aug_patch)
                
                evolve_step = input_search.size(0)
                input_search = input_search.view(-1, *input_search.size()[2:])
                target_bbox = target_bbox.view(-1, *target_bbox.size()[2:])
                gt_bbox = gt_bbox.view(-1, *gt_bbox.size()[2:])
                template = data['template'].unsqueeze(0).repeat(evolve_step, 1, 1, 1, 1).view(-1, *data['template'].size()[1:])
                if 'online_template' in data:
                    online_template = data['online_template'].unsqueeze(0).repeat(evolve_step, 1, 1, 1, 1).view(-1, *data['online_template'].size()[1:])
                
                # the input of post_transform is the `data` dict
                # TODO: optimize this part
                data['det_img'] = input_det
                data['ori_shape'] = orig_shape
                data['target_bbox_det'] = target_bbox_det
                
                data['template'] = template
                data['target_bbox'] = target_bbox
                if 'online_template' in data:
                    data['online_template'] = online_template
                # used for debugging
                data['search_orig'] = data['search']
                data['search'] = input_search
                data['normalize_bbox'] = gt_bbox 
                data['composed_images_pose'] = composed_images_pose
                data = self.post_transform(data)
                
                engine_time, cur_time = time.time() - cur_time, time.time()
                                
                input_dict = dict(
                    template=data['template'],
                    # TODO: [] hard code check here
                    online_template=data['online_template'] if 'online_template' in data else None,
                    search=data['search'],
                    run_score_head=True,
                )
                
                ## Attack Pose Estimator
                pose_heatmaps = self.pose_estimator(data['composed_images_pose'])
                ## Attack Detector
                processed_det_images = self.detector.images_detection_adv(data['det_img'], data['ori_shape'])
                ## Attack Tracker
                out = self.model(**input_dict)
                
                model_time, cur_time = time.time() - cur_time, time.time()
                if self.debug:
                    self._debug(out, processed_det_images, data)
                loss_input_dict = dict(
                    ...
                    gt_bbox=data['normalize_bbox'],
                    target_bbox=target_bbox,
                    pred_bbox_det=processed_det_images,
                    target_bbox_det=target_bbox_det,
                    target_pose=self.target_pose,
                    pred_pose=pose_heatmaps,
                    patch=self.patch,
                )
                ...
                pbar.update(1)
                pbar.desc = f"Epoch {epoch} | data_time: {data_time:.2f} | engine_time: {engine_time:.2f} | " \
    ...
    def test_video(self, video_dataset):
        ...
        ## TODO: modify renderer to engine
            ...
            for frame_idx, data in tqdm(enumerate(video_dataset), total=len(video_dataset), desc="Testing"):
                ...
                if frame_idx == 0:
                    ...
                    if data['apply_attack']:
                        ...
                        img = data['img']
                    # defense applied before input to the DNNs
                    if self.defense is not None:
                        img = self.defense(img)
                    out = self.tracker.track(img)
                    # alarm applied after the prediction
                    alarm = False
                    if self.alarm is not None:
                        alarm = self.alarm(out, img)
    ...
    def _debug(self, out, out_det, data):
        """Debugging function."""
        debug_siamrpn = False
        ...
        gt_bbox = np.array([gt_bbox[0] - gt_bbox[2] / 2, gt_bbox[1] - gt_bbox[3] / 2,
                            gt_bbox[0] + gt_bbox[2] / 2, gt_bbox[1] + gt_bbox[3] / 2])
        gt_bbox = gt_bbox.astype(np.int32)

        target_bbox = data['target_bbox'][0].cpu().numpy() * np.array([w, h, w, h])
        target_bbox = np.array([target_bbox[0] - target_bbox[2] / 2, target_bbox[1] - target_bbox[3] / 2,
                                target_bbox[0] + target_bbox[2] / 2, target_bbox[1] + target_bbox[3] / 2])
        target_bbox = target_bbox.astype(np.int32)
        ...
        search = cv2.rectangle(search, (target_bbox[0], target_bbox[1]), (target_bbox[2], target_bbox[3]), (255, 0, 0), 1)
        ...
        # debug detection
        det_img = data['search_orig'][0].cpu().detach().numpy()
        pred_bbox_det = out_det[out_det[:, 0]==0][:, 1:5].cpu().detach().numpy() # [N, 4] [x1, y1, x2, y2] absolute coordinates
        scores = out_det[out_det[:, 0]==0][:, 5].cpu().detach().numpy()
        # draw all detection boxes
        for bbox, score in zip(pred_bbox_det, scores):
            bbox = bbox.astype(np.int32)
            det_img = cv2.rectangle(det_img, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 0, 255), 1)
            # visualize the score
            det_img = cv2.putText(det_img, f"{score:.2f}", (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 1,
                             (0, 0, 255), 2)
        target_shrink_boox = data['target_bbox_det'][0].cpu().detach().numpy().astype(np.int32) # [cx, cy, w, h]
        det_img = cv2.rectangle(det_img, 
                                (target_shrink_boox[0] - target_shrink_boox[2] // 2, target_shrink_boox[1] - target_shrink_boox[3] // 2),
                                (target_shrink_boox[0] + target_shrink_boox[2] // 2, target_shrink_boox[1] + target_shrink_boox[3] // 2), 
                                (0, 255, 0), 1)
        cv2.imwrite('./debug/det_img.jpg', det_img)
                    
        ## Visualize the pose estimation inputs
        composed_images_pose = data['composed_images_pose'][0].permute(1, 2, 0).cpu().detach().numpy()
        # new_images[:, 0].add_(-0.406)
        # new_images[:, 1].add_(-0.457)
        # new_images[:, 2].add_(-0.480)
        composed_images_pose[:, :, 0] += 0.406
        composed_images_pose[:, :, 1] += 0.457
        composed_images_pose[:, :, 2] += 0.480
        composed_images_pose *= 255
        composed_images_pose = composed_images_pose.astype(np.uint8)
        cv2.imwrite('./debug/composed_images_pose.jpg', composed_images_pose)
